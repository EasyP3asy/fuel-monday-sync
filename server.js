...
const MONITORING_BOARD_ID =  process.env.FUEL_BOARD_ID;
...
async function makeApiRequest({
  ...
  try {
    ...
    if (!response.ok) {
      console.error('API request failed:', response.status, response.statusText, responseData);
      return null; // keep your original behavior (null on error)
      // or: throw new Error(`API request failed: ${response.status} ${response.statusText}`);
    }
